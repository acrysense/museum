...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (historyContent) {
    historyContent.forEach(function (item) {
      ...
      var columnWidth = Number(columns[0].getBoundingClientRect().height);
      ...
      function distributeTextInChunks() {
        ...
        var column = columns[currentColumn].querySelector('.history__column-inner');
        var tempText = remainingText || "";

        // Добавляем слова в текущую колонку, пока не достигнем высоты
        while (index < words.length) {
          ...
          // Проверяем, достигли ли мы максимальной высоты
          if (column.scrollHeight > columnWidth) {
            // Если текст выходит за пределы, обрезаем последнее слово
            tempText = tempText.substring(0, tempText.lastIndexOf(' '));
            column.innerHTML = tempText.replace(/ \|\|BR\|\| /g, '<br><br>');

            // Сохраняем остаток текста для следующей колонки
            remainingText = words[index - 1] + " ";
            currentColumn++;
            distributeTextInChunks(); // Рекурсивно вызываем для следующей колонки
            return;
          }
        }
        remainingText = "";
      ...
      columns.forEach(function (column) {
        var columnInner = column.querySelector('.history__column-inner');
      ...
      //     const height = width * 1.361904761904762;
      //     column.style.height = `${height}px`;
      // });

      // function distributeText() {
      //     for (let currentColumn = 0; currentColumn < 6 && remainingText; currentColumn++) {
      ...
      //             let currentText = columns[currentColumn].innerHTML;
      //             // columns[currentColumn].innerHTML = currentText.replaceAll('||BR||', '');
      //             columns[currentColumn].querySelector('.history__column-inner').innerHTML = currentText.replace(/ \|\|BR\|\|/g, '');
      ...
      //     //     columns[currentColumn].innerHTML = tempText.replace(/ \|\|BR\|\| /g, '<br><br>');
